...
if [ -n "$existing" ]; then
  ...
  ctx="A saved tap already covers ${host} (site: ${site}). Run \`tap list ${site}\` or check tap:// resources to find it, then use mcp__tap__run with the right args instead of WebFetch."
else
  ctx="No saved tap for ${host} yet. WebFetch cannot reach logged-in content here. Capture one with mcp__tap__capture (run /tap:setup first if the site needs login), then replay it at zero tokens."
fi
